...
namespace Nate.Core
{
    public partial class StateMachine<TStateModel> : IStateMachine<TStateModel> where TStateModel : IStateModel
    {
        ...
        private Dictionary<Trigger, List<Transition<TStateModel>>> globalTransitions = new Dictionary<Trigger, List<Transition<TStateModel>>>();
        ...
        public void AddGlobalTransition(Transition<TStateModel> transition)
        {
            ...
            {
                throw new InvalidTransitionException("Global transitions cannot have pre-defined source states as they are, by definition, from any state to a defined target.");
            }
            ...
            lock (globalTransitionsLock)
            {
                ...
                if (globalTransitions[transition.Trigger].Contains(transition))
                {
                    ...
                    throw new InvalidTransitionException(
                        String.Format("State Machine already has a defined global transition to target '{0}' on trigger '{1}'",
        ...
        public void Trigger(Trigger trigger, TStateModel model)
        {
            ...
            {
                throw new InvalidStateModelException("State model's CurrentState object property must be of type State<TStateModel>");
            }

            // get current state
            State<TStateModel> currentState = (State<TStateModel>)model.CurrentState;
            ...
            {
                State<TStateModel> nextState = firstTransitionWithPassingGuard.Target;
                ...
                if (nextState != currentState)
                {
                    ...
                    {
                        Transitioning(this, new TransitionEventArgs<TStateModel>(model, currentState, nextState, trigger));
                    }
                    ...
                    {
                        Transitioned(this, new TransitionEventArgs<TStateModel>(model, currentState, nextState, trigger));
                    }
                }
